...
public class AmazonS3KeyValueRoot implements KeyValueRoot
{
	...
	public AmazonS3KeyValueRoot(
		...
		this(s3, bucket, N5DirectoryPath.of(root), createBucket);
	}
	...
	public synchronized KeyValueAccess getKVA() {
		if (kva == null) {
			kva = new AmazonS3KeyValueAccess(s3, uri().toString(), createBucket);
		}
		return kva;
	}
	...
	@Override
	public URI uri() {
		return URI.create("s3://" + bucket + "/").resolve(root.uri());
	}
	...
	class S3LazyRead implements LazyRead {
		...
		public ReadData materialize(final long offset, final long length) throws N5IOException {
			...
			// NB. The request will not fail if we explicitly requested a range
			// that exceeds the object's actual size, so we explicitly check for
			// that case.
			if (length > 0 && bytes.length < length)
				throw new N5IOException("Requested " + length + " bytes at offset " + offset + " but only received " + bytes.length);
